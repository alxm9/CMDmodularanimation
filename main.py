...
def run_animation(*args): #First arg should be edge/bg
    ...
    clear()
    argslist = tuple_into_list(args) # tuple into ss_obj list. Got a feeling I'm taking a bad approach doing this
    max_frame = max(args, key=lambda ss_obj: ss_obj.frames).frames # max() gives object
    ...
    for objs in zip(*list_framelists): # * unpacks locations list into n different lists (objs)
        for chars in zip(*objs): # objs unpacked into n different characters
            ...
            for i in range(len(chars)): # zlevel: [char, color]
                z_char_color[z_dict[argslist[i]]] = [chars[i], color_dict[argslist[i]]]
...
while True:
    run_animation(dude,backpillars,frontpillars, outline1)
